...
def map_country_to_english(country, country_mapping):
    if isinstance(country, str):  # Only process if the country is a string
        country_cleaned = re.sub(r'\W+', '', country.strip().lower())  # Remove non-alphanumeric chars, lowercase, strip spaces
...
def match_country_isin(df1, df2, country_mapping):
    ...
    df2[['ISIN', 'Matched Værdipapirets navn', 'Matched Udsteder', 'Kommuner']] = df2['Land oversat'].apply(
        lambda country: pd.Series(find_matches(country, df1))
    )
    print('Done')
